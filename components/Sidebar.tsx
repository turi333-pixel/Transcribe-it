...
export const Sidebar: React.FC<SidebarProps> = ({ items, activeId, onSelect, onDelete, onNew }) => {
  return (
    <aside className="w-80 h-full border-r border-slate-200 bg-white flex flex-col shrink-0 shadow-2xl lg:shadow-none">
      <div className="p-4 border-b border-slate-100 flex flex-col gap-4">
        ...
        <button 
          onClick={onNew}
          className="w-full bg-slate-900 text-white font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-colors active:scale-95 shadow-sm"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 no-scrollbar">
        ...
        {items.length === 0 ? (
          <div className="text-center py-10 px-4">
            ...
            <p className="text-sm text-slate-400">History is empty.</p>
          </div>
        ) : (
          items.map(item => (
            <div 
            ...
            >
              <div className="flex justify-between items-start mb-1">
                ...
                <button 
                  ...
                  className="lg:opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 transition-opacity p-0.5"
                >
              ...
              <div className="flex justify-between items-center text-[11px] text-slate-400">
                ...
                <span className="truncate max-w-[100px]">{item.content.substring(0, 30)}...</span>
              </div>
            </div>
          ))
        )}
